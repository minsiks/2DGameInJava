...
//		projectile = new OBJ_Rock(gp);
	...
	private void attacking() {
		...
		if(spriteCounter > 5 && spriteCounter <= 25) {
			...
			int iTlieIndex = gp.cChecker.checkEntity(this, gp.iTile);
			damageInteractiveTile(iTlieIndex);
		...
		if(i != 999) {
			...
			// INVENTORY ITEMS
				...
				if(inventory.size() != maxIventorySize) {
					...
					gp.playSE(1);
					text =  gp.obj[gp.currentMap][i].name + " 를 얻었다!";
				}else {
					text = "더 들수 없어요!";
				}
		...
		if(i != 999) {
			if(gp.monster[gp.currentMap][i].invincible == false) {
				...
				if(gp.monster[gp.currentMap][i].life <= 0) {
					...
					gp.ui.addMessae(gp.monster[gp.currentMap][i].name + "을 잡았습니다!");
					gp.ui.addMessae("경헙치 " + gp.monster[gp.currentMap][i].exp +" +");
